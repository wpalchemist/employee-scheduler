...
<?php if ( is_admin() && '' !== $employee ) { ?>
	<h3>
		<?php
			...
			printf(
				__( 'Schedule for %1$s through %2$s for %3$s', 'employee-scheduler' ),
...
<?php } else { ?>
	<h3><?php printf( __( 'Schedule for %1$s through %2$s', 'employee-scheduler' ), esc_attr( $this->terminal_date( $week, 'start' ) ), esc_attr( $this->terminal_date( $week, 'end' ) ) ); ?></h3>
<?php } ?>

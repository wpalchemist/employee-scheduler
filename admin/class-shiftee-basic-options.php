	...
	class Shiftee_Basic_Options {
		...
		public function add_options_page() {
			add_menu_page(
				...
				plugin_dir_url( __FILE__ ) . '/partials/images/shiftee-icon.png',
				87.2317
			);
			add_submenu_page(
				'shiftee',
				__( 'Instructions', 'employee-scheduler' ),
				__( 'Instructions', 'employee-scheduler' ),
				'manage_options',
				'instructions',
				array(
			...
			add_submenu_page(
				'shiftee',
				__( 'About Shiftee', 'employee-scheduler' ),
				__( 'About Shiftee', 'employee-scheduler' ),
				'manage_options',
				'about-shiftee',
				array(
			...
			add_submenu_page(
				null,
				__( 'Shiftee Upgrades', 'employee-scheduler' ),
				__( 'Shiftee Upgrades', 'employee-scheduler' ),
				'manage_options',
				'shiftee-upgrades',
				array(
					$this,
					'shiftee_upgrades_page',
				)
			);
		...
		public function show_instructions() {
			include SHIFTEE_BASIC_DIR . '/admin/partials/instructions.php';
		}
		...
		public function about_shiftee_page() {
			include SHIFTEE_BASIC_DIR . '/admin/partials/about-shiftee.php';
		}
		...
		public function shiftee_upgrades_page() {
			include SHIFTEE_BASIC_DIR . '/admin/partials/shiftee-upgrades.php';
		}
		...
		public function options_page_init() {
			...
			add_settings_field(
				'admin_notify_clockin',
				__( 'Clockin Notification', 'employee-scheduler' ),
				array( $this, 'admin_notify_clockin_render' ),
				'wpaesm_plugin_options',
				'shiftee_basic_admin_notifications',
				array(
					__( 'Notify admin when a staff member clocks in to a shift', 'employee-scheduler' ),
				)
			);
			...
			add_settings_field(
				'date_format',
				__( 'Date Format', 'employee-scheduler' ),
				array( $this, 'date_format_render' ),
				'wpaesm_plugin_options',
				'shiftee_basic_settings',
				array(
					__( 'Select the date format you want Shiftee to display', 'employee-scheduler' ),
				)
			);

			add_settings_field(
				'time_format',
				__( 'Time Format', 'employee-scheduler' ),
				array( $this, 'time_format_render' ),
				'wpaesm_plugin_options',
				'shiftee_basic_settings',
				array(
					__( 'Select the time format you want Shiftee to display', 'employee-scheduler' ),
				)
			);
		...
		public function date_format_render( $args ) {
			...
			<select name="wpaesm_options[date_format]">
				...
				<option value="M j" <?php selected( $this->options['date_format'], 'M j' ); ?> >Jan 25</option>
				<option value="n/j/Y" <?php selected( $this->options['date_format'], 'n/j/Y' ); ?> >1/25/2018</option>
				<option value="n/j/y" <?php selected( $this->options['date_format'], 'n/j/y' ); ?> >1/25/18</option>
				<option value="n/j" <?php selected( $this->options['date_format'], 'n/j' ); ?> >1/25</option>
				<option value="j F Y" <?php selected( $this->options['date_format'], 'j F Y' ); ?> >25 January 2018</option>
				<option value="j M Y" <?php selected( $this->options['date_format'], 'j M Y' ); ?> >25 Jan 2018</option>
				<option value="j M" <?php selected( $this->options['date_format'], 'j M' ); ?> >25 Jan</option>
				<option value="j-n-Y" <?php selected( $this->options['date_format'], 'j-n-Y' ); ?> >25-1-2018</option>
		 ...
		 * @since 2.1.7
		 */
		public function time_format_render( $args ) {
			?>

			<select name="wpaesm_options[time_format]">
		...
		public function validate_options( $input ) {
			...
			if ( isset( $input['date_format'] ) ) {
				$options['date_format'] = wp_filter_nohtml_kses( $input['date_format'] );
			}

...
if ( ! class_exists( 'Shiftee_Basic_Admin' ) ) {
	class Shiftee_Basic_Admin {
		...
		public function __construct( $plugin_name, $version ) {
			...
			include_once SHIFTEE_BASIC_DIR . '/libraries/Tax-meta-class/Tax-meta-class.php';
		 ...
		 * Announce Shiftee
		 *
		 * Each user will see a splash sceen with basic instructions for using Shiftee and/or new feature announcements
		 *
		 * @since 2.0.0
		 */
		public function announce_shiftee() {
			...
			if ( $this->helper->check_user_role( 'administrator' ) || $this->helper->check_user_role( 'shiftee_manager' ) ) {
				...
				$current_announcement = apply_filters( 'shiftee_current_announcement', 'shiftee_dismiss_shiftee_announcement' );
		...
		public function define_metaboxes() {
			...
			$shift_meta->add_field(
				array(
					'name'            => __( 'Regular Hours', 'employee-scheduler' ),
					'id'              => $prefix . 'reg_hours',
					'type'            => 'hidden',
					'sanitization_cb' => array( $this, 'calculate_reg_hours' ),
				)
			);

			$shift_meta->add_field(
				array(
					'name'            => __( 'OT Hours', 'employee-scheduler' ),
					'id'              => $prefix . 'ot_hours',
					'type'            => 'hidden',
					'sanitization_cb' => array( $this, 'calculate_ot_hours' ),
				)
			);
			...
			$breaks = $shift_meta->add_field(
				array(
					'id'          => $prefix . 'breaks',
					'name'        => __( 'Breaks', 'employee-scheduler' ),
					'type'        => 'group',
					'description' => __( 'Breaks recorded by staff member during this shift', 'employee-scheduler' ),
					'options'     => array(
						'group_title'   => __( 'Break {#}', 'employee-scheduler' ),
						'add_button'    => __( 'Add Another Break', 'employee-scheduler' ),
						'remove_button' => __( 'Remove Break', 'employee-scheduler' ),
						'sortable'      => false,
					),
					'show_on_cb'  => array( $this, 'are_we_tracking_breaks' ),

				)
			);

			$shift_meta->add_group_field(
				$breaks,
				array(
					'name'        => __( 'Break Start', 'employee-scheduler' ),
					'id'          => 'break_start',
					'type'        => 'text_datetime_timestamp',
					'date_format' => $this->helper->safe_date_format(),
					'time_format' => $this->options['time_format'],
			...
			$shift_meta->add_group_field(
				$breaks,
				array(
					'name'        => __( 'Break End', 'employee-scheduler' ),
					'id'          => 'break_end',
					'type'        => 'text_datetime_timestamp',
					'date_format' => $this->helper->safe_date_format(),
					'time_format' => $this->options['time_format'],
			...
			$shift_meta->add_field(
				array(
					'name'            => __( 'Shift Pay Rate', 'employee-scheduler' ),
					'desc'            => __( 'This is the hourly rate for this shift, automatically calculated based on staff member\'s wage settings', 'employee-scheduler' ),
					'id'              => $prefix . 'wage',
					'type'            => apply_filters( 'shiftee_wage_field_type', 'hidden' ),
					'sanitization_cb' => array( $this, 'calculate_wage' ),
					'attributes'      => apply_filters( 'shiftee_wage_field_attributes', '' ),
				)
			);
			...
			$shift_notes_group = $shift_notes->add_field(
				array(
		...
		public function calculate_worked_duration( $field_args, $field ) {
			...
			$duration = $this->helper->get_shift_duration( $shift_id, 'worked' );

			return $duration;
		}

		/**
		 * Calculate the number of regular hours in a shift
		 *
		 * @since 2.1.2
		 *
		 * @param $field_args
		 ...
		 */
		public function calculate_reg_hours( $field_args, $field ) {

			$shift_id  = $field['render_row_cb'][0]->data_to_save['post_ID'];
			$reg_hours = $this->helper->get_shift_reg_vs_ot_hours( $shift_id, 'reg' );
		...
		/**
		 * Calculate the number of overtime hours in a shift
		 *
		 * @since 2.1.2
		 *
		 * @param $field_args
		 ...
		 * @return float|int|mixed
		 */
		public function calculate_ot_hours( $field_args, $field ) {

			$shift_id = $field['render_row_cb'][0]->data_to_save['post_ID'];
			$ot_hours = $this->helper->get_shift_reg_vs_ot_hours( $shift_id, 'ot' );
		...
		/**
		 * Determine whether or not to show the "breaks" field group
		 *
		 * @since 2.1.5
		 *
		 * @return bool
		...
		/**
		 * Make shift overview page sortable by shift date
		 *
		 * @since 2.1.1
		 *
		...
		public function send_notification_email( $postid, $employeeid ) {
			...
			$email->send_email( $to, $cc, $subject, $message );
		}
		...
		/**
		 * Email admin when employee clocks in
		 *
		 * @since 2.1.2
		 *
		 * @param $shift
		...
		public function calculate_wage_on_clock_out( $shift ) {
			...
			$wage = $this->helper->calculate_shift_wage( $shift->ID );

			update_post_meta( $shift->ID, '_shiftee_wage', $wage );

		}

		/**
		 * When an employee clocks out, calculate the worked duration and the regular and overtime hours
		 *
		 * @since 2.1.0
		 *
		 * @param $shift
		...
		public function calculate_duration_on_clock_out( $shift ) {
			...
			$reg_hours = $this->helper->get_shift_reg_vs_ot_hours( $shift, 'reg' );
			update_post_meta( $shift, '_shiftee_reg_hours', $reg_hours );

			$ot_hours = $this->helper->get_shift_reg_vs_ot_hours( $shift, 'ot' );
			update_post_meta( $shift, '_shiftee_ot_hours', $ot_hours );
